...
class TurtleClient(Node):
    def __init__(self):
        ...
        # Declare parameter to set turtle color on startup
        self.declare_parameter('turtleColor','blue', ParameterDescriptor(description='Initial color of the turtle'))
        # Read parameter value and apply to turtle display
        turtleColor = self.get_parameter('turtleColor').get_parameter_value().string_value
        self.turtle_display.color(turtleColor)


        #### pen size parameter ####
        self.declare_parameter('penSize', 2, ParameterDescriptor(description='Initial pen size of the turtle'))
        ...
        # Prepare service request to send color to server
